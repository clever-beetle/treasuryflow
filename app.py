...
def export_pdf():
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.cell(0, 10, 'TREASURY FLOW', 0, 0, 'L')
            self.set_font('Arial', 'B', 11)
            self.set_text_color(108, 117, 125)
            self.cell(0, 10, 'FINANCIAL STATEMENTS REPORT', 0, 1, 'R')
    ...
    pdf.cell(30, 6, 'Prepared For:', 0, 0, 'L')
    pdf.set_font('Arial', '', 10)
    pdf.set_text_color(17, 24, 39)
    pdf.cell(65, 6, str(fullname), 0, 0, 'L')
    
    pdf.set_font('Arial', 'B', 10)
    pdf.set_text_color(17, 35, 126)
    pdf.cell(25, 6, 'Report Date:', 0, 0, 'L')
    pdf.set_font('Arial', '', 10)
    pdf.set_text_color(17, 24, 39)
    ...
    pdf.set_font('Arial', 'B', 10)
    pdf.set_text_color(17, 35, 126)
    pdf.cell(30, 6, 'Period:', 0, 0, 'L')
    pdf.set_font('Arial', '', 10)
    pdf.set_text_color(17, 24, 39)
    pdf.cell(65, 6, f"{start_date or 'All'} to {end_date or 'All'}", 0, 0, 'L')
    
    pdf.set_font('Arial', 'B', 10)
    pdf.set_text_color(17, 35, 126)
    pdf.cell(25, 6, 'Generated By:', 0, 0, 'L')
    pdf.set_font('Arial', '', 10)
    pdf.set_text_color(17, 24, 39)
    pdf.cell(0, 6, 'Fernando Capital Engine', 0, 1, 'L')
    pdf.ln(6)
    ...
    pdf.cell(63, 5, 'TOTAL CASH IN (INCOME)', 0, 0, 'L')
    pdf.cell(63, 5, 'TOTAL CASH OUT (EXPENSE)', 0, 0, 'L')
    pdf.cell(0, 5, 'NET FLOW / SAVINGS', 0, 1, 'L')
    
    pdf.set_font('Arial', 'B', 13)
    pdf.set_text_color(16, 185, 129)
    pdf.cell(63, 8, f"+ Rp {total_income:,.2f}", 0, 0, 'L')
    pdf.set_text_color(239, 68, 68)
    pdf.cell(63, 8, f"- Rp {total_expense:,.2f}", 0, 0, 'L')
    if net_flow < 0:
        pdf.set_text_color(239, 68, 68)
        pdf.cell(0, 8, f"- Rp {abs(net_flow):,.2f}", 0, 1, 'L')
    else:
        pdf.set_text_color(17, 35, 126)
        pdf.cell(0, 8, f"Rp {net_flow:,.2f}", 0, 1, 'L')
    pdf.ln(8)
    ...
    pdf.cell(68, 8, 'DESCRIPTION', 1, 0, 'C', True)
    pdf.cell(32, 8, 'CATEGORY', 1, 0, 'C', True)
    pdf.cell(30, 8, 'ACCOUNT', 1, 0, 'C', True)
    pdf.cell(36, 8, 'AMOUNT', 1, 1, 'C', True)
    ...
    for t in transactions:
        ...
        pdf.cell(24, 7, str(t['date']), 1, 0, 'C', fill)
        
        desc = t['description'] or 'No Description'
        if len(desc) > 35: desc = desc[:32] + "..."
        pdf.cell(68, 7, str(desc), 1, 0, 'L', fill)
        
        pdf.cell(32, 7, str(t['category']), 1, 0, 'L', fill)
        
        clean_account = t['account_name'].split('] ')[1] if '] ' in t['account_name'] else t['account_name']
        pdf.cell(30, 7, str(clean_account), 1, 0, 'L', fill)
        ...
        pdf.cell(36, 7, amt_text, 1, 1, 'R', fill)
        pdf.set_text_color(17, 24, 39)
        fill = not fill

    response = Response(pdf.output(dest='S').encode('latin-1', 'ignore'))
    response.headers.set('Content-Disposition', 'attachment', filename=f"financial_report_{start_date or 'all'}_to_{end_date or 'all'}.pdf")
...
def financial_performance():
    ...
    return render_template('financial_performance.html', 
                           ...
                           assets_list=assets_list, total_asset_value=total_asset_value)
